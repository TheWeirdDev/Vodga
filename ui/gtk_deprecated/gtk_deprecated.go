...
func (statusIcon *StatusIcon) PopupMenu(v *gtk.Menu, button uint, activateTime uint32) {
	if v == nil || v.GObject == nil {
		return
	}
	p := unsafe.Pointer(v.GObject)
	C.gotk_menu_popup_at_status_icon(C.toGtkMenu(p), statusIcon.native(), C.guint(button), C.guint32(activateTime))
}
